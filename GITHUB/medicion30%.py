...
# Enlaces troncales del XML de CORE.
# Cada extremo se define como (router, interfaz).
BACKBONE_LINKS = [
...
def calculate_link_loads(before, after, interval_seconds):
    """Calcula carga direccional de cada enlace troncal en Mbps."""
    loads = []
    ...
    for link in BACKBONE_LINKS:
        ...
        # Para A -> B usamos TX de A y RX de B.
        # Promediamos ambos extremos para reducir pequeñas diferencias de medida.
        a_to_b_tx = max(0, a_after["tx_bytes"] - a_before["tx_bytes"])
        a_to_b_rx = max(0, b_after["rx_bytes"] - b_before["rx_bytes"])
        a_to_b_mbps = ((a_to_b_tx + a_to_b_rx) / 2) * 8 / interval_seconds / 1_000_000

        # Para B -> A usamos TX de B y RX de A.
        b_to_a_tx = max(0, b_after["tx_bytes"] - b_before["tx_bytes"])
        b_to_a_rx = max(0, a_after["rx_bytes"] - a_before["rx_bytes"])
        b_to_a_mbps = ((b_to_a_tx + b_to_a_rx) / 2) * 8 / interval_seconds / 1_000_000
        ...
        loads.append({
            ...
            "max_direction_mbps": max(a_to_b_mbps, b_to_a_mbps)
        })

    return loads


def print_link_load_report(loads):
    """Muestra carga y utilización de todos los enlaces troncales."""
    ordered = sorted(
        loads,
        key=lambda item: item["max_direction_mbps"],
        reverse=True
    )
    ...
    print("==========================================")
    print(
        f"Intervalo de medida: {MEASUREMENT_INTERVAL} segundos | "
        f"Capacidad por dirección: {LINK_CAPACITY_MBPS} Mbps"
    )
    print()

    for item in ordered:
        util_ab = item["a_to_b_mbps"] / LINK_CAPACITY_MBPS * 100
        util_ba = item["b_to_a_mbps"] / LINK_CAPACITY_MBPS * 100

        print(
            f"{item['router_a']}:{item['iface_a']} -> "
            f"{item['router_b']}:{item['iface_b']} | "
            f"{item['a_to_b_mbps']:.3f} Mbps | {util_ab:.2f}%"
        )
        print(
            f"{item['router_b']}:{item['iface_b']} -> "
            f"{item['router_a']}:{item['iface_a']} | "
            f"{item['b_to_a_mbps']:.3f} Mbps | {util_ba:.2f}%"
        )
        print()

    if ordered:
        busiest = ordered[0]
        max_mbps = busiest["max_direction_mbps"]
        max_util = max_mbps / LINK_CAPACITY_MBPS * 100
        suggested_scale = (
            TRAFFIC_SCALE * 30.0 / max_util
            if max_util > 0
            else TRAFFIC_SCALE
        )
        ...
        print("------------------------------------------")
        print(
            "Enlace con mayor carga: "
            f"{busiest['router_a']} - {busiest['router_b']}"
        )
        print(f"Carga máxima direccional: {max_mbps:.3f} Mbps")
        print(f"Utilización máxima: {max_util:.2f}%")
        print(
            "Factor sugerido para aproximarse al 30%: "
...
def run_clients(session_path, nodes, flows):
    ...
    print("Leyendo contadores iniciales antes de lanzar el tráfico...")
    counters_before = read_interface_counters(session_path, ROUTERS)
    ...
    elapsed_launch_time = time.monotonic() - measurement_start
    remaining_time = MEASUREMENT_INTERVAL - elapsed_launch_time

    if remaining_time > 0:
        print(
            f"Esperando {remaining_time:.2f} segundos "
            f"para completar una ventana total de "
            f"{MEASUREMENT_INTERVAL} segundos..."
        )
        time.sleep(remaining_time)
    else:
        print(
            "Aviso: el lanzamiento de todos los flujos tardó más "
            f"de {MEASUREMENT_INTERVAL} segundos."
        )

    print("Leyendo contadores finales de los routers...")
    counters_after = read_interface_counters(session_path, ROUTERS)

    actual_interval = time.monotonic() - measurement_start

    link_loads = calculate_link_loads(
        counters_before,
        counters_after,
        actual_interval
    )
    print_link_load_report(link_loads)
...
def print_link_load_report(summary, actual_interval):
    """Muestra la carga media y el pico observado de cada enlace troncal."""
    ordered = sorted(
        summary,
        key=lambda item: item["peak_total_mbps"],
        reverse=True
    )
    ...
    if ordered:
        busiest = ordered[0]
        max_load = busiest["peak_total_mbps"]
        ...
        print("------------------------------------------")
        print(
            "Enlace con mayor carga total: "
            f"{busiest['router_a']} - {busiest['router_b']}"
        )
        print(f"Utilización máxima observada: {max_util:.2f}%")
...
def run_clients(session_path, nodes, flows):
    ...
    print_link_load_report(link_summary, actual_interval)
...
def stop_servers(session_path, nodes):
    for node in nodes:
        ...
        if result.returncode == 0:
            ...
            print(f"No hay mas procesos activos en {node}")
